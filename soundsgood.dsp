...
// GATE
gate_bp = bp2(checkbox("h:soundsgood/t:expert/h:[1]gate/[1]bypass"),gate);
gate = ef.gate_stereo(gate_thresh,gate_att,gate_hold,gate_rel) with{
  gate_thresh = vslider("h:soundsgood/t:expert/h:[1]gate/[2]thr",-90,-90,0,1);
  gate_att = vslider("h:soundsgood/t:expert/h:[1]gate/[3]att",0,0,0.1,0.01);
  gate_hold = vslider("h:soundsgood/t:expert/h:[1]gate/[4]hold",0.1,0,1,0.1);
  gate_rel = vslider("h:soundsgood/t:expert/h:[1]gate/[5]rel",0.5,0,5,0.1);
};

// EQ with bypass
eq_bp = bp2(checkbox("h:soundsgood/t:expert/h:[4]eq/[1]bypass"),eq);
eq = hp_eq : tilt_eq : side_eq;

// HIGHPASS
hp_eq = par(i,2,fi.highpass(1,freq)) with {
  freq = vslider("h:soundsgood/t:expert/h:[4]eq/h:[1]highpass/[1]freq [unit:Hz] [scale:log]", 5, 5, 1000,1);
};

// TILT EQ STEREO
tilt_eq = par(i,2,_) : par(i,2, fi.lowshelf(N, -gain, freq) : fi.highshelf(N, gain, freq)) with{
    N = 1;
    gain = vslider("h:soundsgood/t:expert/h:[4]eq/h:[2]tilt eq/[1]gain [unit:db]",0,-6,6,0.5);
    freq = vslider("h:soundsgood/t:expert/h:[4]eq/h:[2]tilt eq/[2]freq [unit:Hz] [scale:log]", 630, 200, 2000,1);
};
...
side_eq = midside : (_, eq) : (_*gain_m,_*gain_s) : midside with{
    ...
    gain_m = vslider("h:soundsgood/t:expert/h:[4]eq/h:[3]side eq/[4]gain M [unit:db]",0,-6,6,0.5) : ba.db2linear;
    gain_s = vslider("h:soundsgood/t:expert/h:[4]eq/h:[3]side eq/[5]gain S [unit:db]",0,-6,6,0.5) : ba.db2linear;


};

// LEVELER
leveler_bp = bp2(checkbox("h:soundsgood/t:expert/h:[3]leveler/[1]bypass"),leveler(target));
leveler(target) = B <:    B    ,   (B <: B,B : lk2, + : calc : _ <: B) : ro.interleave(N,2) : par(i,N,*)
    ...
    meter_leveler_gate = vbargraph("h:soundsgood/t:expert/h:[3]leveler/[6]gate",0,1);

    leveler_speed = vslider("h:soundsgood/t:expert/h:[3]leveler/[4]speed", init_leveler_speed, .005, 0.15, .005);
    leveler_gate_thresh = vslider("h:soundsgood/t:expert/h:[3]leveler/[5]gate-thr[unit:dB]", init_leveler_gatethreshold,-90,0,1);
    limit_pos = vslider("h:soundsgood/t:expert/h:[3]leveler/[7]max +", init_leveler_maxboost, 0, 60, 1);
    limit_neg = vslider("h:soundsgood/t:expert/h:[3]leveler/[8]max -", init_leveler_maxcut, 0, 60, 1) : ma.neg;
};

// KNEE COMPRESSOR
kneecomp_bp = bp2(checkbox("h:soundsgood/t:expert/h:[7]knee-comp/[1]bypass"),kneecomp(target));
kneecomp(target) = midside : co.RMS_FBcompressor_peak_limiter_N_chan(strength,thresh,threshLim,att,rel,knee,link,meter,meterLim,2) : midside : post_gain with {
    ...
    strength = vslider("h:soundsgood/t:expert/h:[7]knee-comp/[1]strength", 0.1, 0, 1, 0.1);
    thresh = target + vslider("h:soundsgood/t:expert/h:[7]knee-comp/[unit:dB][2]tar-thr",init_kneecomp_thresh,-12,6,1);
    threshLim = +6; //vslider("threshLim",3,-12,3,1);
    att = vslider("h:soundsgood/t:expert/h:[7]knee-comp/[3]att",0.4,0.001,1,0.001);
    rel = vslider("h:soundsgood/t:expert/h:[7]knee-comp/[4]rel",0.8,0.01,1,0.001);
    knee = vslider("h:soundsgood/t:expert/h:[7]knee-comp/[5]knee",12,0,12,1);
    link = vslider("h:soundsgood/t:expert/h:[7]knee-comp/[6]link", 0.5, 0, 1, 0.1);
    meter = _<: _,(ba.linear2db : ma.neg : vbargraph("h:soundsgood/t:expert/h:[7]knee-comp/[unit:dB]",0,3)) : attach;
    meterLim = _; //_<: _,(ba.linear2db : ma.neg : vbargraph("h:soundsgood/t:expert/h:[7]knee-comp/[unit:dB]",0,3)) : attach;

    //post_gain
    post_gain = par(i,2,_ * g) with {
        g =  vslider("h:soundsgood/t:expert/h:[7]knee-comp/post gain[unit:dB]", init_kneecomp_postgain,-10,+10,0.5) : ba.db2linear;
    };
...
// LIMITER
limiter_bp = bp2(checkbox("h:soundsgood/t:expert/h:[7]limiter/[1]bypass"),limiter);
...
limiter = limiter_lad_N(2,limiter_lad_lookahead, init_limiter_lad_ceil : ba.db2linear, limiter_lad_attack, limiter_lad_hold, limiter_lad_release) : post_gain with{
    ...
    limiter_postgain = vslider("h:soundsgood/t:expert/h:[7]limiter/[3]post-gain[unit:dB]", init_limiter_postgain,-10,+10,0.5) : ba.db2linear;
    limiter_meter = _ <: attach(ba.linear2db : abs : vbargraph("h:soundsgood/t:expert/h:[7]limiter/[2][unit:dB]gr",0,12));
};


// BRICKWALL
brickwall_bp = bp2(checkbox("h:soundsgood/t:expert/h:[8]brickwall/[1]bypass"),brickwall);
...
brickwall = limiter_lad_N(N, limiter_lad_lookahead, limiter_lad_ceil, limiter_lad_attack, limiter_lad_hold, limiter_lad_release) with{
    ...
    brickwall_meter = _ <: attach(ba.linear2db : abs : vbargraph("h:soundsgood/t:expert/h:[8]brickwall/[8][unit:dB]gr",0,12));
...
correlate_meter(x,y) = x,y <: x , attach(y, (corr(t) : meter_correlate_meter )) : _,_ with {
    ...
    meter_correlate_meter = vbargraph("h:soundsgood/t:expert/h:[2]correlation/correlation",-1,1);
};

// stereo correction based on correlation
correlate_correct_bp = bp2(checkbox("h:soundsgood/t:expert/h:[2]correlation/[1]correlate correct bypass"), correlate_correct);
...
mscomp8i_bp = bp2(checkbox("h:soundsgood/t:expert/h:[5]mscomp/h:[0]bypass/[1]bypass"),mscomp8i(target));
...
with {
  ...
  crossoverFreqs = vslider("h:soundsgood/t:expert/h:[5]mscomp/h:[1]lo/[1]freq", 60, 20, 20000, 1),vslider("h:soundsgood/t:expert/h:[5]mscomp/h:[2]hi/[1]freq", 8000, 20, 20000, 1):LogArray(Nr_crossoverFreqs);
  strength_array = vslider("h:soundsgood/t:expert/h:[5]mscomp/h:[1]lo/[2]strength", 0.1, 0, 8, 0.1),vslider("h:soundsgood/t:expert/h:[5]mscomp/h:[1]hi/[2]strength", 0.3, 0, 8, 0.1):LinArray(Nr_bands);
  thresh_array = target + vslider("h:soundsgood/t:expert/h:[5]mscomp/h:[1]lo/[3]thresh", -2, -12, 12, 0.5),target + vslider("h:soundsgood/t:expert/h:[5]mscomp/h:[1]hi/[3]thresh", -6, -12, 12, 0.5):LinArray(Nr_bands);
  att_array = (vslider("h:soundsgood/t:expert/h:[5]mscomp/h:[1]lo/[4]att", 15, 0, 100, 0.1)*0.001,vslider("h:soundsgood/t:expert/h:[5]mscomp/h:[1]hi/[4]att", 0.1, 0, 100, 0.1)*0.001):LogArray(Nr_bands);
  rel_array = (vslider("h:soundsgood/t:expert/h:[5]mscomp/h:[1]lo/[5]rel", 150, 1, 1000, 1)*0.001,vslider("h:soundsgood/t:expert/h:[5]mscomp/h:[1]hi/[5]rel", 25, 1, 1000, 1)*0.001):LogArray(Nr_bands);
  knee_array = (vslider("h:soundsgood/t:expert/h:[5]mscomp/h:[1]lo/[6]knee", 12, 0, 30, 0.1),vslider("h:soundsgood/t:expert/h:[5]mscomp/h:[1]hi/[6]knee", 12, 0, 30, 0.1)):LinArray(Nr_bands);
  link_array = (vslider("h:soundsgood/t:expert/h:[5]mscomp/h:[1]lo/[7]link", 0.5, 0, 1, 0.1),vslider("h:soundsgood/t:expert/h:[5]mscomp/h:[1]hi/[7]link", 0.5, 0, 1, 0.1)):LinArray(Nr_bands);
  mscomp8i_outGain = vslider("h:soundsgood/t:expert/h:[5]mscomp/h:[3]out/[3]output gain", 1, -6, 6, 0.5):ba.db2linear;
