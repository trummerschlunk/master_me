...
leveler_sc(fl,fr,l,r) =
  (calc(lk2(fl,fr),lk2_short(fl,fr),lk2(l,r))
   :((_*(1-bp)+bp)
     <: (_*l,_*r))
  ,_)
with {

  calc(lufs,short_lufs,input_lufs) = FB(lufs,short_lufs,input_lufs)~_: ( ba.db2linear,_);
  FB(lufs,short_lufs,input_lufs,prev_gain) =
    (target - lufs)
    // (raw_target- (overcompression_compensation*factor) - lufs)
    +(prev_gain )
  ...
  with {
    ...
    attack =
      (att / (((speedfactor*(1-undead_att))+undead_att):max(ma.EPSILON)));
    release =
      (rel * (leveler_expander*ma.MAX+1) /
       (((speedfactor*(1-undead_rel))+undead_rel):max(ma.EPSILON))
      );
    // *overcompression_compensation;
    diff = abs(target - lufs);
    speedfactor = (autoSat((diff/(deadzone*0.5))-1)+1)*0.5;
    compression_amount =
      input_lufs+prev_gain-lufs
      : si.smooth(ba.tau2pole(long_length))
      :hbargraph("comp amount[unit:dB]", 0, 12)
    ;
    overcompression_compensation =
      compression_amount-allowed_compression :max(0);
    fax = hslider("fax", 1, 0, 1, 0.001);
    power = hslider("power", 1, 0, 1000, 0.001);
    factor = hslider("factor", 1, 0, 10, 0.001);
    allowed_compression = hslider("allowed_compression", 6, 0, 12, 1);
    raw_target = vslider("v:soundsgood/h:easy/[3]Target[unit:dB][symbol:target]", init_leveler_target,-50,0,1);

    target = raw_target - overcompression_compensation;
    leveler_gate_thresh = raw_target + vslider("v:soundsgood/t:expert/h:[3]leveler/[5][unit:db][symbol:leveler_gate_threshold]leveler gate threshold", init_leveler_gatethreshold,-90,0,1);
    leveler_expander =
...
sc_compressor(fl,fr,l,r,target) =
 target
, (
  (fl,fr,l,r)
...
mscomp_bp(target) = target,bp2(checkbox("v:soundsgood/t:expert/h:[5]mscomp/h:[0]bypass/[0][symbol:mscomp_bypass]mscomp bypass"),
                ms_enc
                               : B_band_Compressor_N_chan(Nba,Nch,target)
                : ms_dec
               ) ;

B_band_Compressor_N_chan(B,N,target) =
  si.bus (N) <: si.bus (2 * N)
...
// LIMITER
limiter_rms_bp(target) = bp2(checkbox("v:soundsgood/t:expert/h:[7]limiter/[0]limiter bypass[symbol:limiter_bypass]"),limiter_rms(target));
limiter_rms(target) = co.RMS_FBFFcompressor_N_chan(strength,thresh,att,rel,knee,0,1,fffb,limiter_meter,2) : post_gain with{
